...
(() => {
  ...
  const cloneV2 = (value) => (typeof structuredClone === 'function' ? structuredClone(value) : JSON.parse(JSON.stringify(value)));
  ...
  async function flushCloudSync() {
    ...
    if (pendingCloudSave && cloudConfigured() && cloudSession && navigator.onLine) {
      try { await syncStateToCloud(); } catch (_) {}
    }
  }
  window.vv2FlushCloudSync = flushCloudSync;
  ...
  async function enhancedSave(notify = true) {
    ...
    if (cloudConfigured() && cloudSession && navigator.onLine) {
      scheduleCloudSync();
      if (notify) toast('Datos guardados; sincronizando…');
    } else if (notify) {
